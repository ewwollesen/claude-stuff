...
| `Mattermost-Plugin-Confluence/` | `~/Repositories/Claude-Repos/Mattermost-Plugin-Confluence/` | Confluence integration plugin — OAuth (3LO + Server), channel subscriptions, Server/Cloud webhooks, Forge bridge polling, @-mention DMs |
| `Mattermost-Plugin-Github/` | `~/Repositories/Claude-Repos/Mattermost-Plugin-Github/` | GitHub integration plugin — OAuth, subscriptions, webhooks, permalinks, review-SLA digest, GitHub Enterprise |
...
For repos where `CLAUDE.md` is NOT tracked upstream (Mattermost, Enterprise, Calls, Boards, Playbooks, Operator, RTCD, Gitlab, LegalHold, Github, Confluence), the symlink is hidden from git via `.git/info/exclude`.
